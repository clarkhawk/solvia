...
interface JoinRequest { id: string; email: string; status: string; createdAt: string; }
...
export default function TeamSettingsPage() {
  ...
  async function load() {
    const [teamResponse, invitationResponse, joinRequestResponse] = await Promise.all([fetch("/api/v1/team"), fetch("/api/v1/invitations"), fetch("/api/v1/join-requests")]);
    if (teamResponse.ok) setUsers(await teamResponse.json());
    if (invitationResponse.ok) setInvitations(await invitationResponse.json());
    if (joinRequestResponse.ok) setJoinRequests(await joinRequestResponse.json());
  }
  useEffect(() => { void load(); }, []);
  ...
  async function reviewJoinRequest(id: string, action: "approve" | "reject") {
    const response = await fetch("/api/v1/join-requests", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, action }) });
    if (!response.ok) { const data = await response.json().catch(() => ({})); setMessage(data.error ?? "Traitement impossible."); return; }
    setMessage(action === "approve" ? "Demande approuvée : le membre peut maintenant accéder à l'entreprise." : "Demande refusée.");
    await load();
  }
  ...
  return <div className="space-y-6">
    ...
    <Card><CardHeader><CardTitle>Demandes pour rejoindre l&apos;équipe</CardTitle><CardDescription>Une demande approuvée donne le rôle commercial, modifiable ensuite dans les permissions.</CardDescription></CardHeader><CardContent className="space-y-3">{joinRequests.filter((request) => request.status === "pending").map((request) => <div key={request.id} className="flex flex-wrap items-center justify-between gap-3 rounded-md border p-3 text-sm"><div><p className="font-medium">{request.email}</p><p className="text-muted-foreground">Demandée le {new Date(request.createdAt).toLocaleDateString("fr-FR")}</p></div><div className="flex gap-2"><Button size="sm" onClick={() => void reviewJoinRequest(request.id, "approve")}>Approuver</Button><Button size="sm" variant="outline" onClick={() => void reviewJoinRequest(request.id, "reject")}>Refuser</Button></div></div>)}{!joinRequests.some((request) => request.status === "pending") && <p className="text-sm text-muted-foreground">Aucune demande en attente.</p>}</CardContent></Card>
